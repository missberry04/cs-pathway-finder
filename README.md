...
Pathway content is fully data-driven (`src/content/pathways/*.ts`), so adding a new field (e.g. game dev, mobile) is a content change, not a UI change.

## Running locally

```bash
npm install
npx prisma db push   # syncs the schema to your DATABASE_URL (Postgres)
npm run dev
```

Requires a `.env` with `DATABASE_URL` (Postgres connection string) and `AUTH_SECRET` (any random string).
